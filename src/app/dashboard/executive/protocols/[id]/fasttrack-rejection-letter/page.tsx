...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const parts = [data?.applicant_surname, data?.serial_text, 'Fast Track Rejected'].filter(Boolean)
  return { title: { absolute: parts.join(' ') } }
}

export default async function FastTrackRejectionLetterPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const supabase = await createClient()
  ...
  if (!protocol.fast_tracked) redirect(`/dashboard/executive/protocols/${id}`)
    ...
    <>
      <style>{`
        @media print {
          ...
          .letter-page * { visibility: visible !important; }
      ...
      <div
        ...
        data-pdf-title={[protocol.applicant_surname, protocol.serial_text?.replace('/', '-'), 'Fast Track Rejected'].filter(Boolean).join(' ')}
      >
        ...
        <div className="letter-body px-10 pt-8 pb-12">
          ...
          <div className="text-sm leading-7 space-y-5 mb-10">
            <p>
              ...
              <strong>fast tracking / Expedited Review.</strong>{' '}
              Following committee review it was determined that the protocol requires your
              attendance (or a nominated proxy) for full review at the next DRC meeting
              {protocol.meeting_date ? ` (${protocol.meeting_date})` : ''}.
            </p>
            <p>
