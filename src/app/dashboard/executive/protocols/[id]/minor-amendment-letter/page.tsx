...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const parts = [data?.applicant_surname, data?.serial_text, 'Minor Amendment'].filter(Boolean)
  return { title: { absolute: parts.join(' ') } }
}

export default async function MinorAmendmentLetterPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const supabase = await createClient()
  ...
  if (protocol.final_outcome !== 'minor_amendment') redirect(`/dashboard/executive/protocols/${id}`)
        ...
        <div className="letter-body px-10 pt-8 pb-12">
          ...
          <div className="text-sm leading-7 space-y-5 mb-10">
            <p>
              ...
              pleased to inform you that the committee approved the scientific merit of the study,{' '}
              <strong>pending</strong> the correction/consideration of the{' '}
              <strong>minor amendments</strong> noted in the attached document.
            </p>
            <p>
              ...
              provide a cover letter with <em>track-changed</em> response the relevant comment. Your final approval letter
              will be issued on receipt of the corrected protocol
            </p>
            <p>
