...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  const parts = [data?.applicant_surname, data?.serial_text, 'Major Amendment'].filter(Boolean)
  return { title: { absolute: parts.join(' ') } }
}

export default async function AmendmentLetterPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const supabase = await createClient()
  ...
  if (!profile || (profile.role !== 'executive' && profile.role !== 'admin')) redirect('/dashboard')
  ...
  if (protocol.final_outcome !== 'major_amendment') redirect(`/dashboard/executive/protocols/${id}`)
  ...
  return (
    <>
      ...
      <div
        ...
        data-pdf-title={[protocol.applicant_surname, protocol.serial_text?.replace('/', '-'), 'Major Amendment'].filter(Boolean).join(' ')}
      >
        ...
        <div className="letter-body px-10 pt-8 pb-12">
          ...
          <div className="text-sm leading-7 space-y-5 mb-10">
            <p>
              The above proposal has been reviewed by the Department of Surgery Research Committee.
              Please could you address the comments made in the tracked change document with respect to
              the study.
            </p>
            <p>
              Please include the above project reference number on resubmission of your corrected protocol.
              Should you wish to dispute the comments and recommendations provided by the DRC, please
              provide a cover letter with <em>track-changed</em> response the relevant comment. Your final approval
              letter will be issued on receipt of the corrected protocol
            </p>
            <p>
              Committee members are available and willing to assist you with any queries regarding this
              protocol.
            </p>
          </div>
